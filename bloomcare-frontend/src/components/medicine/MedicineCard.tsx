// bloomcare-frontend/src/components/medicine/MedicineCard.tsx
...
export const MedicineCard: React.FC<MedicineCardProps> = ({
  ...
  const linkPath = isAuthenticated 
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg hover:border-[#22c55e]/30 transition-all duration-200">
      <Link to={linkPath} className="block relative">
        ...
        />
        {isInCart && (
          <div className="absolute top-2 right-2 bg-[#d1f843] text-black text-xs font-bold px-2 py-1 rounded-xl flex items-center gap-1">
      ...
      <div className="p-3 md:p-4">
        <Link to={linkPath} className="block group">
          <h3 className="font-semibold text-black group-hover:text-[#22c55e] transition-colors truncate text-sm md:text-lg font-outfit">
            {medicine.name}
          </h3>
        </Link>
        {medicine.genericName && (
          <p className="text-xs text-gray-500 mt-0.5 font-outfit truncate">{medicine.genericName}</p>
        )}
        ...
        <div className="mt-2 md:mt-3 flex justify-between items-center">
          {showPrice && price !== undefined ? (
            <span className="font-bold text-sm md:text-lg text-black font-outfit">
              ${price.toFixed(2)}
            </span>
          ) : (
            <span className="text-xs md:text-sm text-gray-500 font-outfit">Price varies</span>
          )}
          {showAddButton && onAddToCart && (
            <button
              onClick={handleAddToCart}
              disabled={!inStock}
              className={`px-2 md:px-4 py-1 md:py-2 rounded-xl text-xs md:text-sm font-semibold transition-all duration-200 flex items-center gap-1 font-outfit ${
                isInCart
